...
create table public.outcomes (
  ...
  voided     boolean not null default false,   -- undo path; never delete labels
  voided_by  uuid references public.profiles (id),
  voided_at  timestamptz
);

-- Ranker audit (Phase 5): one row per training run, model card included.
...
-- ---------------------------------------------------------------------------
-- Phase 5 (The Loop): RLS for ranker_runs + atomic outcome RPCs + vector match
-- ---------------------------------------------------------------------------
alter table public.ranker_runs enable row level security;
create policy ranker_runs_select on public.ranker_runs
  for select to authenticated using (true);
-- ranker_runs writes: training notebook via service key only.
